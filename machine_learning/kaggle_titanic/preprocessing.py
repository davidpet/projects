"""Preprocessing for kaggle titanic project."""
...
#       If it were a production pipeline it would be essential, but it's not as
#       urgent since it's a notebook I will only use for a little bit longer.

INPUT_FOLDER = '/mnt/p/Datasets/titanic'
...
def load_data(filename: str = TRAINING_DATA,
              batch_size: int = DEFAULT_BATCH_SIZE,
              shuffle: bool = True,
              shuffle_seed: int = 4,
              label_name: str = 'Survived') -> tf.data.Dataset:
    """
    Load a CSV dataset.
    ...
    Args:
        ...
        label_name(str): Label to use from the dataset columns
            (None to not use a label).
    ...
    dataset = tf.data.experimental.make_csv_dataset(
        filename,
        shuffle=shuffle,
        # Want consistent randomization so I can compare models.
        shuffle_seed=shuffle_seed,
        # Leave the epoch counting to model.fit instead since no data
        # augmentation needed.
        num_epochs=1,
        # Ground truth (turns output from dict into tuple).
...
def transform_columns(batch, labels=None):
    ...
    Args:
        ...
        labels: The label values for the batch, if present.
            This will be None for inference.
    ...
    new_batch = {
        'Class':
            batch['Pclass'],
        'Sex':
            tf.cast(batch['Sex'] == b'male', tf.int32),
        'Siblings':
            batch['SibSp'],
        'Origin':
            datasets.remap_column_entries(batch['Embarked'], embark_to_class),
        'AgeGroup':
            int(batch['Age'] // 10),  # found experimentally
        'FareGroup':
            int(batch['Fare'] // 50),  # found experimentally
    }
...
def extract_passenger_ids(batch):
    ...
    Because this is meant to be used for test data,
    it will not work if you have labels in the dataset.
    """
...
def prepare_training_data_for_model(
    ...
    normalized_dataset = raw_dataset.map(transform_columns).map(
        datasets.named_column_oh_expander(ONE_HOT_SPECS)).map(
            datasets.named_column_scaler(scalers)).map(datasets.to_matrix())

    return datasets.split(normalized_dataset,
                          num_batches=DATASET_SIZE / batch_size)


# TODO: Consider common helpers with prepare_training_data_for_model
...
def prepare_test_data_for_model(
    ...
    The shape of the PassengerId tensor will be (num_samples, 1) for easier
    concatenating.
    Because the dataset will not be shuffled, the two tensors can be iterated
    in parallel safely.
    """

    # I could have also used PassgnerId as the label as a hack,
    # but I decided to keep it cleaner.
    raw_dataset = load_data(filename=TEST_DATA,
                            batch_size=batch_size,
                            shuffle=False,
                            label_name=None)
    scalers = load_scalers()
...
# TODO: Consider ways to make this more general later
#       (eg. break apart the csv part, etc.).
def test_model_and_create_csv(model: tf.keras.Model,
                              index_dataset: tf.data.Dataset,
                              input_dataset: tf.data.Dataset):
    """
    Run predictions using model in batches from input_dataset and create output
    csv with index_dataset entries as 1st column.
    """
    ...
    with open(OUTPUT_DATA, mode='w', newline='') as file:
        for batch_ids, batch in zip(index_dataset, input_dataset):
            batch_predictions = tf.cast(
                tf.reshape(model.predict_on_batch(batch) >= 0.5, (-1, 1)),
                tf.int32)
            batch_output = tf.concat([batch_ids, batch_predictions], axis=1)  # pylint: disable=unexpected-keyword-arg,no-value-for-parameter

            df = pd.DataFrame(batch_output.numpy(),
