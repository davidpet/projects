...
class DatasetsTests(tf.test.TestCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        self.labels = tf.constant([[1, 0], [0, 1], [1, 1]])
        ...
        self.labeled_dataset = tf.data.Dataset.from_tensor_slices(
            (self.tensor, self.labels))
        ...
        self.dictionary_dataset = tf.data.Dataset.zip(
            make_dictionary_dataset(self.dictionary))
        self.labeled_dictionary_dataset = tf.data.Dataset.zip(
            (self.dictionary_dataset,
             tf.data.Dataset.from_tensor_slices(self.labels)))
    ...
    def test_show_first_batch_with_dictionary(self):
        ...
        self.assertAllEqual(list(self.dictionary.keys()),
                            list(self.printed_objects[0].keys()))
        self.assertAllEqual(list(self.first_dictionary_batch.values()),
                            list(self.printed_objects[0].values()))
    ...
    def test_split_with_default_ratio(self):
        training, validation = datasets.split(self.tensor_dataset,
                                              num_batches=3)
    ...
    def test_safe_batch_transformer_with_labels(self):
        transformed = self.labeled_dataset.map(
            datasets.safe_batch_transformer(lambda batch: 'gotcha!'))
    ...
    def test_safe_batch_transformer_without_labels(self):
        transformed = self.tensor_dataset.map(
            datasets.safe_batch_transformer(lambda batch: 'gotcha!'))
    ...
    def test_named_column_scaler(self):
        ...
        actual = next(
            iter(
                self.dictionary_dataset.map(
                    datasets.named_column_scaler(scalers))))
    ...
    def test_named_column_scaler_with_labels(self):
        ...
        actual = next(
            iter(
                self.labeled_dictionary_dataset.map(
                    datasets.named_column_scaler(scalers))))
    ...
    def test_named_column_oh_expander(self):
        ...
        dataset = self.dictionary_dataset.map(
            lambda d: {k: tf.cast(d[k], tf.int32) for k in d})
    ...
    def test_train_named_scalers(self):
        ...
        self.assertAllEqual(tf.reshape(self.dictionary['field2'], (3, 2, 1)),
                            scalers['field2'].partial_fit_objects)
        self.assertAllEqual(tf.reshape(self.dictionary['field3'], (3, 2, 1)),
                            scalers['field3'].partial_fit_objects)
    ...
    def test_to_matrix_specific_type(self):
        transformed = self.dictionary_dataset.map(
            datasets.to_matrix(dtype=tf.int32))

        self.assertEqual(list(transformed)[0].dtype, tf.int32)
        self.assertAllEqual(tf.cast(self.tensorized_dictionary, tf.int32),
                            list(transformed))
