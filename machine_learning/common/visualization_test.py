...
class VisualizationTests(tf.test.TestCase):
    ...
    def test_exmain_candidate_column(self):
        with patch.object(pd.Series, 'plot') as fake_plot:
            ...
            visualization.examine_candidate_column(self.dataframe, 'col1', 'Column 1', 'col2', 'Column 2', lambda s: self.capture_series(s))
    ...
    def test_compute_truth_ratio_with_even_split(self):
        self.assertEqual(visualization.compute_truth_ratio(self.dataframe, 'col2'), 0.5)

    def test_compute_truth_ratio_with_skew(self):
        self.dataframe.at[1, 'col2'] = 1

        self.assertEqual(visualization.compute_truth_ratio(self.dataframe, 'col2'), 0.75)
    
    def test_compute_truth_ratio_with_zero(self):
        self.dataframe.at[0, 'col2'] = 0
        self.dataframe.at[3, 'col2'] = 0

        self.assertEqual(visualization.compute_truth_ratio(self.dataframe, 'col2'), 0.)
    
    def test_compute_truth_ratio_with_one(self):
        self.dataframe.at[1, 'col2'] = 1
        self.dataframe.at[2, 'col2'] = 1

        self.assertEqual(visualization.compute_truth_ratio(self.dataframe, 'col2'), 1.)
        ...
        with patch('matplotlib.pyplot.plot') as fake_plot:
            ...
            with patch('matplotlib.pyplot.xlabel') as fake_xlabel:
                ...
                with patch('matplotlib.pyplot.title') as fake_title:
                    ...
                    visualization.graph_training_stats(VisualizationTests.FakeHistory({
                        'loss': 'Loss Value',
                        'accuracy': 'Accuracy Value',
                        'val_accuracy': 'Validation Accuracy Value',
                    }))
        self.assertAllEqual([
            'Loss Value', 'Loss', 'Epoch', 
            'Accuracy Value', 'Accuracy', 'Epoch', 
            'Validation Accuracy Value', 'Validation Accuracy', 'Epoch',
        ], self.plotted_objects)
    
    def test_print_matrix_with_tf(self):
        with patch('builtins.print') as fake_print:
            fake_print.side_effect = lambda *args, **kargs: self.capture_print(*args, **kargs)
            visualization.print_matrix(tf.constant([[1, 2], [3, 4]]))
        self.assertAllEqual(['1', 'tab', '2', 'tab', 'newline', '3', 'tab', '4', 'tab', 'newline',], self.printed_objects)
