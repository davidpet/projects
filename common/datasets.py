...
def split(dataset: tf.data.Dataset, num_batches:float, training_ratio:float = 0.8) -> Tuple[tf.data.Dataset, tf.data.Dataset]:
    ...
    Args:
        dataset (tf.data.Dataset): The dataset.
        num_batches(float): The total number of batches in the dataset (can be complete or partial).
        training_ratio(float): The ratio of training batches to total batches.

    Returns:
        Tuple of (training, validation) datasets with training containing the first batches rounded down from applying the ratio.
    """
...
def named_column_scaler(scalers: dict):
    ...
    def batch_column_scaler(batch: dict):
        ...
        # This needs to be a function to capture feature_name by value in the loop below.
...
def to_matrix(dtype=tf.float32):
    ...
    The order of columns will match the order of dictionary keys.  Elements will be cast to make them compatible.
    """
    ...
    for element in dataset:
        ...
        if labels != None:
            if labels_tensor == None:
                ...
                labels_tensor = tf.concat([labels_tensor, labels], axis=0)
    if labels_tensor == None:
        return data_tensor
    else:
