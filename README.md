...
- For `Safron`, run `bazel run //machine_learning/safron`. It will ask you for a debate topic, a number of rounds, a filename, etc. For the time being, the file must be an **absolute path** (and cannot contain `~` or anything like that). If you don't give a file, it will not write a file. Either way, the results will show at the console.
